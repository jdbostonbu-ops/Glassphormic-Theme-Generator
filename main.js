...
function createFusionEngine() {
    let _themeA = THEMES.find(t => t.id === 'glass');
    let _themeB = THEMES.find(t => t.id === 'brutal');
    let _ratio = 0.5;
    const _listeners = [];
...
/* ─────────────────────────────────────────────────────────────
   CODE PANEL — render fused tokens as CSS
   ──────────────────────────────────────────────────────────── */
...
function init() {
    ...
    // Brand input — textContent for safety
    ...
    function updateBrand() {
        const raw = brandInput.value.trim() || 'ATLAS';
        msTitle.textContent = raw;
        msLogo.textContent = raw.charAt(0).toUpperCase() || 'A';
    ...
    engine.onChange((fused) => {
        ...
        const url = document.getElementById('previewUrl');
        if (url) {
            const brand = (brandInput.value.trim() || 'atlas').toLowerCase().replace(/\s+/g, '-');
